...
if(is.null(lastTimestamp)){
  lastTimestamp = 1512104400000 # this is the timestamp for dec 1 2017. This is before makerdao launched, so the first few days might not return data.
  newFile = TRUE
...
repeat{
  ...
  queryString <- list(
    ...
    endDate = lastTimestamp + 5184000000, # We attempt to get one day of data at a time, but if there are too many transactions in a day we won't get them all in one call
    direction = "ascending",
  ...
  responseData <- tryCatch({
    fromJSON(content(response, "text"))$payload$data
  },
  error = function(cond) {
    message("Bad Gateway. Trying request again.")
    NULL
  })
  if(is.null(responseData)){
    next
  }
