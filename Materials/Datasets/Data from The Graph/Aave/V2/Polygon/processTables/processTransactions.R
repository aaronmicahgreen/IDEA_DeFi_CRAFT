...
borrows <- rawBorrows %>%
  ...
  left_join(aliases, by = c("onBehalfOfID" = "id")) %>%
  rename(onBehalfOfAlias = alias) %>%
  select(id, type, timestamp, user, userAlias, onBehalfOfID, onBehalfOfAlias, pool, reserve, amount, amountUSD, amountETH, borrowRate, borrowRateMode) %>%
  drop_na() %>%
  distinct()
...
deposits <- rawDeposits %>%
  ...
  left_join(aliases, by = c("onBehalfOfID" = "id")) %>%
  rename(onBehalfOfAlias = alias) %>%
  select(id, timestamp, type, amount, amountUSD, amountETH, reserve, user, userAlias, onBehalfOfID, onBehalfOfAlias, pool) %>%
  drop_na() %>%
  distinct()
...
redeems <- rawRedeems %>%
  ...
  left_join(aliases, by = c("onBehalfOfID" = "id")) %>%
  rename(onBehalfOfAlias = alias) %>%
  select(id,timestamp, type, amount, amountUSD, amountETH, reserve, user, userAlias, onBehalfOfID, onBehalfOfAlias, priceInUsd, pool) %>%
  drop_na() %>%
  distinct()
...
fileName <- "polygonLendingAddresses.csv"
newFile = FALSE
...
for(address in lendingAddressesToCollect$user){
  ...
  response <- VERB("GET", url, 
                   add_headers('x-amberdata-blockchain-id' = 'polygon-mainnet', 
...
cleanedTransactions <- cleanedTransactions %>%
  mutate(userAlias = case_when(is.na(userAlias) & !is.na(user) ~ "Smart Contract",
                               TRUE ~ userAlias),
         onBehalfOfAlias = case_when(is.na(onBehalfOfAlias) & !is.na(onBehalfOfID) ~ "Smart Contract",
  ...
  left_join(addressTypes, by = c("onBehalfOfID" = "address")) %>%
